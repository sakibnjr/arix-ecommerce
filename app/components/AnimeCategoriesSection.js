  ...
  return (
    <section className="relative py-8 sm:py-12 lg:py-16 bg-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Enhanced Anime Categories Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 sm:gap-6 lg:gap-8">
          {animeCategories.map((anime, index) => (
            ...
            >
              ...
              >
                <div
                  className={`relative p-6 sm:p-8 min-h-[280px] sm:min-h-[320px] flex flex-col justify-between overflow-hidden h-full`}
                >
                  ...
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${anime.color} opacity-70`}
                  />

                  {/* Header */}
                  <div className="relative z-10">
                    <h3
                      className={`font-display text-lg sm:text-xl font-bold leading-tight ${anime.textColor} mb-2`}
                    >
                      {anime.name}
                    </h3>
                    <p
                      className={`text-gray-200 text-xs sm:text-sm ${anime.textColor} opacity-90 leading-relaxed`}
                    >
                      {anime.description}
                    </p>
                  </div>
                  ...
                  {/* Enhanced CTA */}
                  <span
                    className={`relative z-10 inline-flex items-center bg-white/20 backdrop-blur-sm ${anime.textColor} px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg text-xs sm:text-sm font-medium group-hover:bg-white/30 transition-all duration-300 tracking-wide border border-white/20`}
                  >
                    View Collection
