...
export default function CheckoutForm({ isOpen, onClose, orderType = "cart" }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.phone.trim()) {
      ...
      const isValid =
        /^01\d{9}$/.test(normalized) || /^\+8801\d{9}$/.test(normalized);
      if (!isValid) {
        newErrors.phone =
          "Please enter a valid Bangladesh phone number (01XXXXXXXXX or +88 01XXXXXXXXX)";
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        toast.success(
          `Thank you, ${formData.fullName}! Your order for ${totalItems} item${
            totalItems > 1 ? "s" : ""
          } has been submitted. We&apos;ll contact you soon to arrange delivery to ${
            formData.city
          } for ৳${finalTotal.toFixed(2)} (including ৳80 shipping).`,
          { duration: 8000 }
        );
      }, 1000);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 bg-white border-b border-gray-200 shadow-sm">
          <h3 className="font-display text-heading-lg text-gray-900 mb-4">
            Order Summary
          </h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                />
                {errors.fullName && (
                  <p className="text-red-500 text-body-sm mt-1">
                    {errors.fullName}
                  </p>
                )}
              </div>
